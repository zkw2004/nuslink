...
def _rank_segments(
    *,
    unique_participant_ids: list[str],
        ...
        for start_minute, end_minute in zip(
            ...
            segments.append(
                RankedMeetupSuggestion(
                    id=f"{day_of_week}-{start_minute}-{end_minute}",
                    label=label,
                    sub=sub,
    ...
    deduped: dict[tuple[int, int, int], RankedMeetupSuggestion] = {}
    for segment in segments:
        key = (segment.day_of_week, segment.start_minute, segment.end_minute)
